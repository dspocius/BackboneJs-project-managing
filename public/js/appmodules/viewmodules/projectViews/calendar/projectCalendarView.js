define([
    ...
    'tpl!./projectCalendarGrid.html',
	'../../../../lib/fullcalendar/lib/moment.min',
	'../../../../lib/fullcalendar/fullcalendar.min'
], function (app,BaseRowView, templateHelpers,Marionette, projectGridRowView, projectGrid) {
    'use strict';
						...
						dataTasksThat[j].reccurence == ''){
							...
							if(dataTasksThat[j].from != ''){
								atdata = {
									...
									textarea_notify: dataTasksThat[j].textarea_notify,
									notify: dataTasksThat[j].notify,
									start: fromEnd,
									end: startTo,
									model_id: if_of_the_model
								};
							}else{
							...
							var textarea_notify = dataTasksThat[j].textarea_notify;
							var notify = dataTasksThat[j].notify;
							if(recc.charAt(0) === 'W'){
								...
								if(wasInside){
									if(estim != ''){
										...
										var dateToHour = currentDateHours;
											...
											atdata = {
												title: abbout,
												textarea_notify: textarea_notify,
												notify: notify,
												model_id: if_of_the_model,
		...
		$('#calendar').fullCalendar({
			...
			eventRender: function (event, element) {
				element.click(function() {
					var th_model = allcollection_th.get(event.model_id);
					if(typeof th_model != "undefined" && th_model != ""){
						var vv = new BaseRowView({model: th_model});
						var ev_title = "<div class='what_is_working_on_this_only_tasks'>Working: <a href='/#/entry/"+event.notify+"'>"+event.notify+"</a></div>"+"<h4>"+event.title+"</h4><div style='white-space: pre; border-bottom:1px solid #ccc;'>"+event.textarea_notify+"</div>";
						vv.showImageAndOtherMultimedia(th_model, ev_title);
					}
				});
			}
		});
