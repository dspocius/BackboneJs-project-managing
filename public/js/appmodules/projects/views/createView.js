define([
...
], function( app, config, templateHelpers, Marionette, templ ) {
	'use strict';
    ...
    return Marionette.ItemView.extend({
		...
		onChangeType: function(){
			...
			var colorSelect = this.model.get('entryColor');
			$('#color_this').val(colorSelect);
			if(radioSelected === 'project'){
				var colorOnSel = this.model.get('color');
				$('#color_this').val(colorOnSel);
			}
			if(radioSelected === 'list'){
				var colorOnSell = this.model.get('listsColor');
				$('#color_this').val(colorOnSell);
			}
		},
		onCreatePress: function(e){
			...
			var textVall = $(this.ui.text).val();
						var regex = /(<([^>]+)>)/ig;
						if(!config.let_add_html_data_title){
							textVall = textVall.replace(regex, "");
						}
