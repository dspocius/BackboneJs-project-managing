...
define(['./module'], function (services) {
    'use strict';
    services.factory('viewChannelMessagesGrid', ['common', 'view', function (common, view) {
        return function(data, scope, grid){
            var viewGrid = function(){
                this.name = "viewChannelMessagesGrid";
				...
				htmlIn += '<div class="message_text_left">{{msg.firstname}}: {{msg.message}}<div ng-bind-html="msg.files | sanitize"></div> <div class="datetimehere" style="font-size:10px;">{{msg.date}}<div></div></div></div></div>';
                        '';
                    htmlIn+='</div>';
