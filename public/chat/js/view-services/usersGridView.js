...
if(typeof noentries != 'undefined' && noentries != ''){
	noentrieText = noentries;
                    ...
                    var htmlIn = '<input type="search" class="form-control input-lg auth-biginput search_textarea" ng-model="qq" placeholder="'+"{{'Filter' | translate}}"+'" />' +
                        ...
                        '<div id="{{user._id}}" class="userChoose {{user.isChosen}} {{user.hasWrote}}" ng-focus="user.setChosen()" ng-click="user.setChosen()">' +

						'<div ng-show="user.pic == '+"''"+'" class="message_from_left_for_photo"><img class="message_from_left miniChatListPhoto" src="/files/brand/logo.png" alt=""></div>'+
						'<div ng-show="user.pic != '+"''"+'" class="message_from_left_for_photo"><img class="message_from_left miniChatListPhoto" src="/files/{{user.email}}/{{user.pic}}" alt=""></div>'+
                        
						'<div class="usersNameBv"><span class="connected" ng-if="user.isConnected != '+"''"+'">' +
                            '<span class="glyphonline glyphicon glyphicon-ok-sign"></span>' +
                        '</span>' +
