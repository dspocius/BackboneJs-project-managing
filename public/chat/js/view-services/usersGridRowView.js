...
define(['./module'], function (services) {
    'use strict';
    services.factory('viewUsersGridRow', ['common', 'viewRow', '$location', function (common, viewRow, $location) {
        return function(data, scope, parent){
            var viewUsersGrid = function(){
                this.setChosen = function(){
					if ($(window).width() > 770 || location.href.indexOf("/p/") > -1) {
						common.trigger('user:change',this);
					} else {
						$location.url('/p/'+this._id);
					}
                }
