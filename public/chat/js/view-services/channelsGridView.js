...
if(typeof noentries != 'undefined' && noentries != ''){
	noentrieText = noentries;
                    ...
                    var htmlIn = '<input ng-if="'+mypScope+'.models.length !== 0" type="search" class="form-control input-lg auth-biginput search_textarea" ng-model="qqchannels" placeholder="'+"{{'Filter' | translate}}"+'" />' +
                        ...
                        '<div id="channel{{channel._id}}" class="userChoose {{channel.hasWrote}}" ng-focus="channel.setChosen()" ng-click="channel.setChosen()">' +
                        '<span class="channel_show" ng-if="channel.show && !channel.Added">' +
                            '<span id="archiveTheHere{{channel._id}}" ng-click="hideChannel(channel._id)" class="glyphicon glyphicon-minus-sign glyph_delete_conv"></span> ' +
                        '</span>' +
                        ...
                        '</span>' +
                        '<div class="aboutChannelHere"><b ng-show="channel.type == '+"'Open'"+'">#</b> <b ng-show="channel.type == '+"'Private'"+'"><span class="glyphicon glyphicon-lock"></b></span> {{channel.name}}</div><b class="channelDescHere">{{channel.about}}</b><div class="bottom"></div>' +
                        addToHtmLChildM+
                        '</div>'+
                        '</span>';
                    htmlIn+='</div>';
