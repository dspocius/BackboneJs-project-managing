...
define([
    ...
    'views/EmptyView',
    'models/logging'
], function (jquery, Backbone, Marionette, NotifyRegion, DialogRegion, Nav, MenuView, Footer,Project, User, EmptyView, logging) {
	'use strict';
    ...
    var menuNew = [{title: '<div class="glyphicon glyphicon-circle-arrow-right icon-in-menu icon-turn-off" aria-hidden="true"></div>', name: 'right',actionEx:'rightClick', active: false}];
        ...
        for(var i=0; i < cachedCollectionResources.length; i++){
            for(var j=0; j < cachedCollectionResources[i].models.length; j++){
                if(resource.get('_id') == cachedCollectionResources[i].models[j].get('_id')){
                    cachedCollectionResources[i].models[j] = resource;
                    not_updated = false;
                }
            }
        }
	...
	app.vent.on('top:leftmenu:show', function () {
        ...
        logged.fetch().success(function(data){
            ...
            userConnected.fetch().success(function(data2){
                ...
                menuNew = [{title: '<div class="glyphicon glyphicon-circle-arrow-right icon-in-menu icon-turn-off" aria-hidden="true"></div>', name: 'right',actionEx:'rightClick', active: false}];
